...
from depth_cov.data.depth_resize import pyr_depth
...
class ImageGradientModule(nn.Module):
  ...
  def forward(self, x):
    gx =  nn.functional.conv2d(nn.functional.pad(x, (1,1,1,1), mode='reflect'),
                            self.kernel_x, groups=x.shape[1])

    gy =  nn.functional.conv2d(nn.functional.pad(x, (1,1,1,1), mode='reflect'),
