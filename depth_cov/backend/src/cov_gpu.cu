  ...
  torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> K12) 
{
  ...
  if (i < x1.size(1) && j < x2.size(1)) {
    ...
    // Square diff
    scalar_t diff_x_sq = diff_x * diff_x;
    scalar_t diff_y_sq = diff_y * diff_y;
    // Sum covariance
...
torch::Tensor cross_covariance_gpu(torch::Tensor x1, torch::Tensor E1, torch::Tensor x2, torch::Tensor E2, float scale) {
  ...
  // const dim3 blocks(NUM_BLOCKS(num_points1), NUM_BLOCKS(num_points2), batch_size);
  ...
  torch::PackedTensorAccessor32<float,3,torch::RestrictPtrTraits> l_ni) 
{
  ...
  for (int i = 0; i < N; i++) { // looping over columns
    if (i == n) {
      ...
      // printf("%d %d: 1.0/%f * (%f - %f x %f)\n", n, i, L[b][n][n], k_ni[b][n][0], L[b][n][i], l_ni[b][i][0]);
      // printf("Solution %d is %f\n", n, l_ni[b][n][0]);
    }
    __syncthreads();
    if (i < n) {
      sum -= L[b][n][i] * l_ni[b][i][0];
      // printf("%d %d: Subtracting %f x %f)\n", n, i, L[b][n][i], l_ni[b][i][0]);
    }
  }
}
  ...
  int N) 
{
  ...
  if (n == 0) {
    ...
    // printf("Hello from b %f, i %f, j %f\n", k_ii, sum_of_squares, l_ii[b][0][0]);
  }
}
...
void get_new_chol_obs_info_gpu(
  ...
  // const int n = obs_info.size(1);
  const int d = obs_info.size(2);

  auto opts = L.options();
  // torch::Tensor obs_info_new = torch::empty({batch_size, 1, d}, opts);

  // const dim3 blocks_train(batch_size);
  // const dim3 threads_train(THREADS);

  // torch::Tensor l_ni = at::linalg_solve_triangular(L, k_ni, /*upper=*/false, /*left=*/true);
  // torch::Tensor l_ii = at::sqrt(k_ii - at::sum(at::square(l_ni), /*dim=*/1, /*keepdim=*/true));
  // torch::Tensor l_ii_inv = 1.0/l_ii;
  
  // torch::Tensor l_ni = torch::zeros({batch_size, L.size(1), 1}, opts);
  // forward_sub_kernel<<<blocks_train, N>>>(
  //   k_ii,
  //   L.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
  //   k_ni.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
  //   l_ni.packed_accessor32<float,3,torch::RestrictPtrTraits>());
  
  // torch::Tensor l_ii = torch::empty({batch_size, 1, 1}, opts);
  // torch::Tensor l_ii_inv = torch::empty({batch_size, 1, 1}, opts);
  // assert(N < THREADS); // Assuming can be done in shared memory
  // solve_chol_diag_kernel<<<blocks_train, threads_train>>>(
  //   k_ii,
  //   l_ni.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
  //   l_ii.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
  //   l_ii_inv.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
  //   N);

  // TODO: Remove sum_of_squares, make shared memory and init to 0 if n==0
